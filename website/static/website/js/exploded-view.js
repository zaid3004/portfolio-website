const cardData = {
    ...
    'Portfolio Website': {
        ...
        description: 'OTT-style portfolio site built with Django, featuring responsive design, Netflix-style layout, and dynamic content sections.',
        sections: {
            'Skills Applied': [
...
function openModal(key) {
    ...
    (data.links || []).forEach(link => {
        linksContainer.insertAdjacentHTML('beforeend', `<a href="${link.url}" class="modal-button" target="_blank" rel="noopener noreferrer">${link.text}</a>`);
    });
...
function initializeModalSystem() {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (card) {
            const rawTitle = (card.querySelector('.movie-title')?.textContent || card.dataset.card || '').trim();
            const mapped = keyMap[normalizeKey(rawTitle)];
            if (mapped) openModal(mapped);
            else console.warn('No cardData entry for movie-title:', rawTitle);
            return;
        }
    });
